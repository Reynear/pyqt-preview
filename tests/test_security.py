...
class TestValidatePythonFile:
    ...
    def test_validate_python_file_directory(self):
        """Test validating directory instead of file."""
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create a directory with .py extension (unusual but possible)
            dir_path = Path(temp_dir) / "script.py"
            dir_path.mkdir()

            try:
                with pytest.raises(SecurityError) as exc_info:
                    validate_python_file(str(dir_path))
                assert "not a file" in str(exc_info.value)
            finally:
                pass  # Directory will be cleaned up by TemporaryDirectory

    @patch("os.access")
    def test_validate_python_file_not_readable(self, mock_access):
        """Test validating Python file that is not readable."""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".py", delete=False) as f:
            f.write("print('Hello')")
            file_path = f.name
    ...
    def test_validate_python_file_returns_resolved_path(self):
        """Test that validate_python_file returns resolved absolute path."""
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create a Python file
            file_path = Path(temp_dir) / "script.py"
...
class TestValidateUIFile:
    """Test cases for validate_ui_file function."""

    def test_validate_ui_file_valid_string_path(self):
        """Test validating a valid UI file with string path."""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".ui", delete=False) as f:
            f.write('<?xml version="1.0" encoding="UTF-8"?><ui></ui>')
            file_path = f.name
    ...
    def test_validate_ui_file_valid_path_object(self):
        """Test validating a valid UI file with Path object."""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".ui", delete=False) as f:
            f.write('<?xml version="1.0" encoding="UTF-8"?><ui></ui>')
            file_path = Path(f.name)
    ...
    def test_validate_ui_file_non_ui_extension(self):
        """Test validating file with non-UI extension."""
        ...
        try:
            with pytest.raises(SecurityError) as exc_info:
                validate_ui_file(file_path)
            assert "must be a UI file" in str(exc_info.value)
            assert file_path in str(exc_info.value)
        finally:
            Path(file_path).unlink()

    def test_validate_ui_file_uppercase_extension(self):
        """Test validating file with uppercase .UI extension."""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".UI", delete=False) as f:
            f.write('<?xml version="1.0" encoding="UTF-8"?><ui></ui>')
            file_path = f.name
    ...
    def test_validate_ui_file_no_extension(self):
        """Test validating file with no extension."""
        with tempfile.NamedTemporaryFile(mode="w", delete=False) as f:
            f.write('<?xml version="1.0" encoding="UTF-8"?><ui></ui>')
            file_path = f.name
    ...
    def test_validate_ui_file_nonexistent(self):
        """Test validating nonexistent UI file."""
        nonexistent_path = "/nonexistent/form.ui"

        with pytest.raises(SecurityError) as exc_info:
            validate_ui_file(nonexistent_path)
        assert "does not exist" in str(exc_info.value)
        assert nonexistent_path in str(exc_info.value)
